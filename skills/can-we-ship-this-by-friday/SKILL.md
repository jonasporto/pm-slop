...
1. **What is left.** Run the `how-are-we-doing-on-this` recipe and take its Next checklist. Count
...
2. **How fast this branch has moved.** Sessions are bursts of commits. A gap
   ...
   `how-are-we-doing-on-this` counted, not raw commits.
...
- When "yes, if" cuts something, cut items, not quality. Do not propose
  ...
  optional.
- Do not run the test suite to look thorough. `how-are-we-doing-on-this` already decided whether
