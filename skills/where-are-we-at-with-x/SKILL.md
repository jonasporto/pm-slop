...
# where-are-we-at-with-x (/wawat)

The question is slop. The answer is not. `how-are-we-doing-on-this` (/hawd) looks at what is in front of
you. `where-are-we-at-with-x` (/wawat) has to find X first, say what it found, and only then report.
...
For everything else: run the `how-are-we-doing-on-this` recipe against the candidate branch
...
- Same evidence rules as `how-are-we-doing-on-this`: Done needs a hash, Next needs a source or the
  `(inferred)` label.
